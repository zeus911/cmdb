...
ace.handle_side_menu = function($) {
	...
	$(document).on(ace.click_event+'.ace.submenu', '.sidebar .nav-list', function (ev) {
		...
		if( !link_element.hasClass('dropdown-toggle') ) {//it doesn't have a submenu return
            ...
            $('.sidebar .nav-list li').removeClass(' active'); // add common
            link_element.parents('li').addClass('active'); // add common
//            $('.sidebar .nav-list li').addClass('open'); // add common
		...
		if( sub.scrollHeight == 0 ) {
		  $(parent_ul).find('> .open > .submenu').each(function() {
			...
			if(this != sub && !$(this.parentNode).hasClass('active')) {
                ...
                $(this).parent().removeClass('open'); // add common
